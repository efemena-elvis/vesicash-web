<template>
  <div>
    <div class="body-area">
      ...
      <div class="form-group" v-if="$route.name === 'GroupBeneficiary'">
        <FormFieldInput
          label_title="Saved amount (Optional)"
          label_subtitle="Enter an amount if payment is regular"
          label_id="amount"
          input_type="number"
          is_required
...
export default {
  ...
  methods: {
    ...
    async addNewBeneficiary() {
      if (this.$route.name === "GroupBeneficiary") {
        this.addToBeneficiaryGroup();
      }
      // SINGLE BENEFICIARY ENTRY
      else {
        const account_found = this.beneficiary_list.find(
        ...
        if (!account_found) {
          const response = await this.handleDataRequest({
            action: "addNewBank",
            payload: this.getBankDetails,
            btn_text: "Save beneficiary",
            alert_handler: {
              success: "Beneficiary account saved successfully",
              error: "Unable to save beneficiary account",
              request_error: "Unable to save beneficiary account",
            },
          });

          if (response.code === 201) {
            this.$emit("closeTriggered");
            setTimeout(() => this.reloadPageData(), 2000);
          }
        } else {
          this.pushToast("Beneficiary account already created", "warning");
          return false;
        }
      }
    },

    async addToBeneficiaryGroup() {
      const response = await this.handleDataRequest({
      ...
      if (response.code === 201) {
        setTimeout(() => this.reloadPageData(), 1000);
      }
    },
